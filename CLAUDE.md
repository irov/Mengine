...
> `AGENTS.md` and `CLAUDE.md` are mirrors of each other: identical content, read by different agents. Any edit to one must be applied to the other in the same commit.
...
- Run and verify apps through Mengine-MCP instead of relying on OS-level screenshots: use `app_launch` with `mode: "hidden_render"`, `frame_capture`, `logs_read`, and `script_call` against an app's own MCP handler when one is available, for every test run made while working. Keep the MCP client connection open for the app's whole lifetime (a one-shot call that exits stops the app, since its lifetime is tied to the client connection). `app_launch` with `mode: "visible"` has been unreliable in this environment: the app can exit on its own a few seconds after connecting (a clean "finish log success" shutdown, not a crash) even though a plain, directly launched build stays open normally; launch the built app directly (e.g. `open path/to/App.app`) for a session meant to stay on the user's screen.
