...
{
	//////////////////////////////////////////////////////////////////////////
	SoundDecoderConverterFFMPEGToOGG::SoundDecoderConverterFFMPEGToOGG( CodecServiceInterface * _service, InputStreamInterface * _stream, LogSystemInterface * _logSystem, FileServiceInterface * _fileService )
	...
	const SoundCodecDataInfo* SoundDecoderConverterFFMPEGToOGG::getCodecDataInfo() const
	{
		...
		{
			LOGGER_ERROR(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG ::ERROR You must set options to converter before asking codec info  " );
		}
	...
	bool  SoundDecoderConverterFFMPEGToOGG::seek( float _timing )
	{
		if( m_decoder == NULL )
		{
			LOGGER_ERROR(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG seek:: ERROR  bad initalisation status  " );
			return false;
		}
	...
	float  SoundDecoderConverterFFMPEGToOGG::timeTell()
	{
		if( m_decoder == NULL )
		{
			LOGGER_ERROR(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG timeTell:: ERROR  bad initalisation status  " );
			return 0.0f;
		}
	...
	unsigned int SoundDecoderConverterFFMPEGToOGG::decode( unsigned char* _buffer, unsigned int _bufferSize )
	{
		if( m_isValid == false)
		{
			LOGGER_ERROR(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG :: ERROR  bad initalisation status  " );
			return 0;
		}

		if(m_dataInfo.size <= 1)
		{
			LOGGER_ERROR(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG :: ERROR  stream is not initialised  " );
			return 0;
		}
		if( _bufferSize != m_dataInfo.size )
		{
			LOGGER_ERROR(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG :: ERROR  buffer size not equal real stream size  " );
			return 0;
		}
	...
	void SoundDecoderConverterFFMPEGToOGG::_invalidate()
	{
		...
		{
			LOGGER_ERROR(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG :: input file name is empty " );
			return;
		}
				
		String inputFileFullPath = m_options.inputFileFullPath;

		FileSystemInterface * fileSystem = m_fileService->getFileSystemInterface();
		int pointPos = inputFileFullPath.find_last_of(".");
		
		String outputFile = inputFileFullPath.substr( 0 , pointPos );
		outputFile += ".ogg";

		if( fileSystem->existFile( outputFile ) == false )
		{
			if( convert_( inputFileFullPath, outputFile ) == false)
			{
				LOGGER_ERROR(m_logSystem)( "Error: Can't convert  sound file '%s'"
		...
		else
		{
			...
			m_convertedStream = fileSystem->openInputStream( outputFile );
		...
		{
			LOGGER_ERROR(m_logSystem)( "Error: Cannot open converted  ogg file '%s'"
				, outputFile.c_str()
				);
		}
		...
		if( m_decoder == NULL )
		{
			LOGGER_ERROR(m_logSystem)( "Error: Can't create sound decoder for converted file '%s'"
				, outputFile.c_str() 
				);
			fileSystem->closeInputStream(m_convertedStream);
			return;
	...
	bool SoundDecoderConverterFFMPEGToOGG::convert_( const String & input, const String & output )
	{
		...
		LOGGER_INFO(m_logSystem)( "SoundDecoderConverterFFMPEGToOGG:: conferting file  '%s' to '%s' "
			, input.c_str()
			, output.c_str() 
			);
