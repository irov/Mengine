...
int
PyDict_MergeFromSeq2(PyObject *d, PyObject *seq2, int override_)
{
    ...
    for (i = 0; ; ++i) {
        ...
        if (override_ || PyDict_GetItem(d, key) == NULL) {
...
int
PyDict_Merge(PyObject *a, PyObject *b, int override_)
{
    ...
    if (PyDict_Check(b)) {
        ...
        for (key = PyIter_Next(iter); key; key = PyIter_Next(iter)) {
            if (!override_ && PyDict_GetItem(a, key) != NULL) {
                Py_DECREF(key);
                continue;
            }
