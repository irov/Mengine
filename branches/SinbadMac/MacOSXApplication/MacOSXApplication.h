...
public:
	...
	virtual bool						registerLogger( OutputStreamInterface * _logger );
	virtual void						unregisterLogger( OutputStreamInterface * _logger );
};
...
public:
	...
	virtual DynamicLibraryInterface *	load( const String & _filename );
	virtual void						unload( DynamicLibraryInterface * _lib );
    ...
    virtual bool openUrlInDefaultBrowser( const String & _url );
    //Program must work even game is shutdown 
    virtual bool executeProgram( const String & _programPath );
    
    static std::string                  s_getDefaultLanguage( void );
};
