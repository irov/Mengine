...
JNIEXPORT int JNICALL SDL_JAVA_INTERFACE(nativeRunMain)(JNIEnv *env, jclass cls, jstring library, jstring function, jobject array)
{
    ...
    SDL_library_handle = dlopen(library_file, RTLD_GLOBAL);

    if (SDL_library_handle == NULL) {
        ...
        if (library_name && *library_name) {
            library_name += 1;
            SDL_library_handle = dlopen(library_name, RTLD_GLOBAL);
        }
    }
    ...
    if (SDL_library_handle) {
        ...
        SDL_main = (SDL_main_func)dlsym(SDL_library_handle, function_name);
        if (SDL_main) {
            int i;
            int argc;
            int len;
...
JNIEXPORT int JNICALL SDL_JAVA_INTERFACE(nativeLoopMain)(JNIEnv *env, jclass cls, jstring function)
{
    ...
    if (SDL_library_handle) {
        ...
        __android_log_print(ANDROID_LOG_ERROR, "SDL", "nativeLoopMain(): Couldn't load library");
    }
