...
new_arena(void)
{
    ...
    if (unused_arena_objects == NULL) {
        ...
        arenaobj = (struct arena_object *)_python_realloc(arenas, nbytes);
        if (arenaobj == NULL)
            return NULL;
        arenas = arenaobj;
...
#else
    address = _python_malloc(ARENA_SIZE);
    err = (address == 0);
#endif
    if (err) {
...
redirect:
    ...
    if (nbytes == 0)
        nbytes = 1;
    return (void *)_python_malloc(nbytes);
}
...
PyObject_Realloc(void *p, size_t nbytes)
{
 ...
 redirect:
     ...
     */
    if (nbytes)
        return _python_realloc(p, nbytes);
