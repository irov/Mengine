...
namespace Mengine
{
    ...
    PyObject * PythonScriptLogger::py_write( pybind::kernel_interface * _kernel, PyObject * _args, PyObject * _kwds )
    {
        ...
        if( HAS_OPTION( "tracescriptlog" ) == true )
        {
            ...
            LOGGER_VERBOSE_LEVEL( STRINGIZE_STRING_LOCAL( "python" ), level, LFILTER_NONE, color, function, lineno ).operator()( "%s", m_messageCache.c_str() );
        }
        else
        {
            LOGGER_VERBOSE_LEVEL( STRINGIZE_STRING_LOCAL( "python" ), level, LFILTER_NONE, color, nullptr, 0 ).operator()( "%s", m_messageCache.c_str() );
        }
