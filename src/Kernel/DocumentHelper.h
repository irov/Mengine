...
#   define MENGINE_DOCUMENT_FACTORABLE [this](const Mengine::Char * _file, const Mengine::Char * _function, uint32_t _line) { Mengine::DocumentPtr doc = Mengine::Helper::makeFactorableUnique<Mengine::Document>(nullptr, _file, _function, _line); doc->message( "%s [==>] %s[%d]", dynamic_cast<const Documentable *>(this)->getDocument()->c_str(), _function, _line ); doc->setParent( dynamic_cast<const Documentable *>(this)->getDocument() ); return doc;}(MENGINE_CODE_FILE, MENGINE_CODE_FUNCTION, MENGINE_CODE_LINE)
#   define MENGINE_DOCUMENT_FACTORABLE_PTR(Ptr) [Ptr](const Mengine::Char * _file, const Mengine::Char * _function, uint32_t _line) { Mengine::DocumentPtr doc = Mengine::Helper::makeFactorableUnique<Mengine::Document>(nullptr, _file, _function, _line); doc->message( "%s [==>] %s[%d]", dynamic_cast<const Documentable *>(Ptr)->getDocument()->c_str(), _function, _line ); doc->setParent( dynamic_cast<const Documentable *>(Ptr)->getDocument() ); return doc;}(MENGINE_CODE_FILE, MENGINE_CODE_FUNCTION, MENGINE_CODE_LINE)
