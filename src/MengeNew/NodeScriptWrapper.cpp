...
namespace Menge
{
	namespace ScriptMethod
	{
		...
		static void setCamera2DDirection( float x, float y )
		{
			...
			//++it)
			//{
			//	SceneNode2D * child = dynamic_cast<SceneNode2D*>(it);
		...
		static mt::vec2f screenToLocal( const std::string& _layerName, const mt::vec2f& _point )
		{
			...
			Node * node = 
				scene->getChildren( _layerName, false );

			if( Layer2D * layer = dynamic_cast<Layer2D*>(node) )
			{
				const mt::vec2f & factor = layer->getParallaxFactor();
				...
				mt::vec2f local = viewport.begin + _point;
		...
		static mt::vec2f getScreenPosition( Node * _node )
		{
			Camera2D * camera = Holder<Player>::hostage()
				->getRenderCamera2D();
			...
			mt::vec2f screen_pos = _node->getScreenPosition( viewport );
	...
	REGISTER_SCRIPT_CLASS( Menge, Node, Base )
	{
		...
		pybind::interface_<Node, pybind::bases<Allocator2D> >("Node", false)
...
*/
		...
		pybind::def( "getViewport", &ScriptMethod::getViewport );
		...
		pybind::def( "getScreenPosition", &ScriptMethod::getScreenPosition );
