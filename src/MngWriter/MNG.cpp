...
namespace mnglib
{
	...
	bool	mngWriter::createMNG(const std::string& _filename)
	{
		int num = 0;
	...
	void	mngWriter::writeFrame(int _index, int _delay)
	{
		assert(_index >= 0);
		assert(_delay >= 0);

		fwrite(&_index, sizeof(unsigned int), 1, m_fp);
		fwrite(&_delay, sizeof(unsigned int), 1, m_fp);
