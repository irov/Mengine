...
FixedJoint::FixedJoint(RigidBodyInterface* _body, const mt::vec2f& _anchor)
{
	body1 = _body;
	body2 = 0;

	float c = cosf(body1->rot), s = sinf(body1->rot);
	mt::vec2f v = _anchor - body1->x;
	localAnchor = mt::vec2f(c * v.x + s * v.y, c * v.y - s * v.x );
	mainAnchor = _anchor;
...
void	FixedJoint::calculate(float inv_dt)
{
	body1->wake();
	...
	r1.x =  cos * localAnchor.x - sin * localAnchor.y;
	r1.y =  sin * localAnchor.x + cos * localAnchor.y;
	...
	float	a = body1->m_1 + I_1y * r1.y;
	float	b = -I_1y * r1.x;
	float	c = -I_1y * r1.x;	
	float	d = body1->m_1 + body1->I_1 * r1.x * r1.x;
	float	det = a * d - b * c;
	...
	bias = (mainAnchor - body1->x - r1) * -0.1f * inv_dt;
	accumulatedImpulse *= relaxation;
...
void	FixedJoint::integrate()
{
	body1->wake();

	mt::vec2f dv = - body1->v - mt::perp(r1) * body1->w;
