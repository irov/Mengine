...
namespace Mengine
{
    ...
    bool Win32Platform::_runService()
    {
        ...
        auto support_message = []( const Char * isa_feature, bool is_supported )
        {
            ...
            LOGGER_VERBOSE_LEVEL( STRINGIZE_STRING_LOCAL_I( MENGINE_LIBRARY_NAME ), LM_MESSAGE_RELEASE, LFILTER_NONE, color, nullptr, 0 )("%s: %s"
                , isa_feature
                , is_supported == true ? "+" : "-"
                );
        };
...
#error "Platform not supported!"
    ...
    bool Win32Platform::createProcess( const Char * _process, const Char * _command, bool _wait, uint32_t * const _exitCode )
    {
        ...
        if( _wait == true )
        {
            ...
            if( exitCode != 0 )
            {
                ...
                if( successful == TRUE )
                {
                    LOGGER_VERBOSE_LEVEL( STRINGIZE_STRING_LOCAL_I( MENGINE_LIBRARY_NAME ), LM_ERROR, LFILTER_NONE, LCOLOR_RED, nullptr, 0 )("%s"
                        , tempFileBuffer
                        );
                }
                else
                {
                    LOGGER_VERBOSE_LEVEL( STRINGIZE_STRING_LOCAL_I( MENGINE_LIBRARY_NAME ), LM_ERROR, LFILTER_NONE, LCOLOR_RED, nullptr, 0 )("invalid read file '%ls'"
                        , tempFileNameBuffer
                        );
                }
    ...
    //////////////////////////////////////////////////////////////////////////
    bool Win32Platform::isShowKeyboard() const
    {
        return true;
    }
    //////////////////////////////////////////////////////////////////////////
