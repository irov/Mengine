...
namespace Menge
{
	...
	bool ImageConverterDDSToHTF::convert()
	{
		...
		MemoryCacheBufferInterfacePtr data_buffer = Helper::createMemoryCacheBuffer( m_serviceProvider, data_full_size, "ImageConverterDDSToHTF_data" );
		...
		size_t encode_byte = encoder->encode( miplevel_data_memory, data_full_size, &htfDataInfo );
