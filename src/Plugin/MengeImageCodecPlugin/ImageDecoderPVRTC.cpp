...
size_t ImageDecoderPVRTC::decode( void * _buffer, size_t _bufferSize )
{
	...
	unsigned int read = static_cast<unsigned int>(m_stream->read(_buffer, _bufferSize));
...
bool ImageDecoderPVRTC::_initialize()
{
	...
	unsigned int nSizeX= m_pvrtc_ptr.width;
	unsigned int nSizeY = m_pvrtc_ptr.height;

	int compressedImageSize = ( _MAX(nSizeX, PVRTC4_MIN_TEXWIDTH) * _MAX(nSizeY, PVRTC4_MIN_TEXHEIGHT) * 4 + 7) / 8;
