...
@implementation AppleAppTrackingPlugin
...
- (void)updateTrackingStatus:(ATTrackingManagerAuthorizationStatus)status {    
    switch(status) {
        case ATTrackingManagerAuthorizationStatusAuthorized: {
            ...
            self.m_idfa = [AppleAppTrackingPlugin makeIDFA];
        }break;
...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[AppleAppTrackingPlugin class]];
    });
...
- (void)authorization {
    ...
    __weak AppleAppTrackingPlugin * weakSelf = self;
    
    [iOSDetail addDidBecomeActiveOperationWithCompletion:^(void (^ _Nonnull completion)(void)) {
        AppleAppTrackingPlugin * strongSelf = weakSelf;
