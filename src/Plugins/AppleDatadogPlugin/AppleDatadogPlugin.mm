...
+ (instancetype) sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[AppleDatadogPlugin class]];
    });
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    NSString * AppleDatadogPlugin_ClientToken = [AppleBundle getPluginConfigString:@PLUGIN_BUNDLE_NAME withKey:@"ClientToken" withDefault:nil];
    
    if (AppleDatadogPlugin_ClientToken == nil) {
    ...
    NSString * AppleDatadogPlugin_Service = [AppleBundle getPluginConfigString:@PLUGIN_BUNDLE_NAME withKey:@"Service" withDefault:bundleIdentifier];
    
    DDConfiguration * configuration = [[DDConfiguration alloc] initWithClientToken:AppleDatadogPlugin_ClientToken
    ...
    configuration.service = AppleDatadogPlugin_Service;
    
    NSString * AppleDatadogPlugin_Site = [AppleBundle getPluginConfigString:@PLUGIN_BUNDLE_NAME withKey:@"Site" withDefault:@"us1"];
    
    if ([AppleDatadogPlugin_Site isEqual:@"us1"] == YES) {
        configuration.site = [DDSite us1];
    } else if ([AppleDatadogPlugin_Site isEqual:@"us3"] == YES) {
        configuration.site = [DDSite us3];
    } else if ([AppleDatadogPlugin_Site isEqual:@"eu1"] == YES) {
        configuration.site = [DDSite eu1];
    } else {
        IOS_LOGGER_ERROR(@"plugin %@ Site %@ not found", @PLUGIN_BUNDLE_NAME, AppleDatadogPlugin_Site);
    ...
    DDLoggerConfiguration * loggerConfiguration = [[DDLoggerConfiguration alloc]
                                                   initWithService:AppleDatadogPlugin_Service
                                                   name:nil
