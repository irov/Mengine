...
+ (instancetype)sharedInstance {
    static AppleFacebookApplicationDelegate * sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[AppleFacebookApplicationDelegate class]];
    });
...
- (BOOL)login:(NSArray<NSString *> *)permissions {
    ...
    __weak AppleFacebookApplicationDelegate * weakSelf = self;
    ...
    [self.m_loginManager logInFromViewController:viewController configuration:configuration completion:^(FBSDKLoginManagerLoginResult * _Nullable result, NSError * _Nullable error) {
        AppleFacebookApplicationDelegate * strongSelf = weakSelf;
