...
@implementation AppleUserMessagingPlatformPlugin

+ (instancetype)sharedInstance {
    static AppleUserMessagingPlatformPlugin * sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[AppleUserMessagingPlatformPlugin class]];
    });
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    __weak AppleUserMessagingPlatformPlugin * weakSelf = self;
    ...
    [UMPConsentInformation.sharedInstance requestConsentInfoUpdateWithParameters:parameters completionHandler:^(NSError * _Nullable error) {
        AppleUserMessagingPlatformPlugin * strongSelf = weakSelf;
        ...
        [iOSDetail addDidBecomeActiveOperationWithCompletion:^(void (^ _Nonnull completion)(void)) {
            AppleUserMessagingPlatformPlugin * strongSelf2 = weakSelf;
