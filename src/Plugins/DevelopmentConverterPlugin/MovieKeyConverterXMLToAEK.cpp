...
namespace Mengine
{
    ...
    {
        //////////////////////////////////////////////////////////////////////////
        static Clipper2Lib::PathD makeClipperPath( const Polygon & _polygon )
        {
            Clipper2Lib::PathD path;
            path.reserve( _polygon.size() );

            for( const mt::vec2f & point : _polygon )
            {
        ...
        //////////////////////////////////////////////////////////////////////////
        static Polygon makeMenginePolygon( const Clipper2Lib::PathD & _path )
        {
            Polygon polygon;
            polygon.reserve( _path.size() );

            for( const Clipper2Lib::PointD & point : _path )
            {
                polygon.append( mt::vec2f( (float)point.x, (float)point.y ) );
            }

            return polygon;
        }
        //////////////////////////////////////////////////////////////////////////
        static void appendClipperPolygon( const Clipper2Lib::PolyPathD & _node, VectorGeolygon * const _output )
        {
            if( _node.IsHole() == false && _node.Polygon().empty() == false )
            {
                Polygon outer = Detail::makeMenginePolygon( _node.Polygon() );
                VectorPolygon holes;

                for( const std::unique_ptr<Clipper2Lib::PolyPathD> & child : _node )
                {
                    if( child->IsHole() == true )
                    {
                        holes.emplace_back( Detail::makeMenginePolygon( child->Polygon() ) );
                    }
                }

                _output->emplace_back( std::move( outer ), std::move( holes ) );
            }

            for( const std::unique_ptr<Clipper2Lib::PolyPathD> & child : _node )
            {
                Detail::appendClipperPolygon( *child, _output );
            }
        }
        //////////////////////////////////////////////////////////////////////////
        static bool clipPolygons( Clipper2Lib::ClipType _clipType, const Polygon & _subject, const Polygon & _clip, VectorGeolygon * const _output )
        {
            Clipper2Lib::PathsD subjects{Detail::makeClipperPath( _subject )};
            Clipper2Lib::PathsD clips{Detail::makeClipperPath( _clip )};
            Clipper2Lib::PolyTreeD solution;

            Clipper2Lib::BooleanOp( _clipType, Clipper2Lib::FillRule::NonZero, subjects, clips, solution, 6 );
            Detail::appendClipperPolygon( solution, _output );
    ...
    {
        MemoryInterfacePtr framePack = Helper::readStreamArchiveMagicMemory( _stream, m_archivator, GET_MAGIC_NUMBER( MAGIC_AEK ), GET_MAGIC_VERSION( MAGIC_AEK ), MENGINE_DOCUMENT_FACTORABLE );

        return framePack != nullptr;
    }
    //////////////////////////////////////////////////////////////////////////
...
#undef MENGINE_WRITE_FRAME_SOURCE
            ...
            ++it )
        {
                ...
                ++it_shape )
            {
                ...
                if( subtract == false )
                {
                    if( Detail::clipPolygons( Clipper2Lib::ClipType::Intersection, polygon, imagePolygon, &output ) == false )
                    {
                ...
                {
                    if( Detail::clipPolygons( Clipper2Lib::ClipType::Difference, imagePolygon, polygon, &output ) == false )
                    {
                        LOGGER_ERROR( "layer [%u] shapes invalid"
                            , layerIndex
                        );
                ...
                else
                {
                    ...
                    for( const Geolygon & geolygon : output )
                    {
                        ...
                        {
                            uint32_t inner_count = (uint32_t)inner.size();
                    ...
                    for( const Geolygon & geolygon : output )
                    {
                        ...
                        for( p2t::Triangle * tr : triangles )
                        {
                            ...
                            uint32_t i0 = (uint32_t)StdIterator::distance( pb, p0 );
                            uint32_t i1 = (uint32_t)StdIterator::distance( pb, p1 );
                            uint32_t i2 = (uint32_t)StdIterator::distance( pb, p2 );
            ...
            ++it )
        {
            ...
            uint32_t polygon_size = (uint32_t)polygon.size();
    ...
    bool MovieKeyConverterXMLToAEK::writeFramePackage_( const Data & _buffer )
    {
        ...
        if( Helper::writeStreamArchiveMagic( output_stream, m_archivator, GET_MAGIC_NUMBER( MAGIC_AEK ), GET_MAGIC_VERSION( MAGIC_AEK ), false, buffer_memory, buffer_size, EAC_BEST ) == false )
        {
