...
namespace Mengine
{
    ...
    bool VideoConverterFFMPEGToOGVA::convert()
    {
        ...
        String buffer = "-loglevel error -y -threads 4 -i \"" + full_input + "\" -vf \"split [a], pad=iw:ih*2 [b], [a] alphaextract, [b] overlay=0:h:alpha=premultiplied\" -vcodec libtheora -f ogg -map_metadata -1 -an -q 10 -pix_fmt yuv420p \"" + full_output + "\" -max_muxing_queue_size 1024";
