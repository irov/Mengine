...
namespace Mengine
{
    ...
    void MovieSlot::_destroy()
    {
        ...
        {
            LOGGER_ERROR( "movie slot '%s' type '%s' has children name '%s' type '%s'!! (please remove, before release movie '%s')"
                , this->getName().c_str()
                , this->getType().c_str()
    ...
    void MovieSlot::_hierarchyChangeParent( Node * _oldParent, Node * _newParent )
    {
        ...
        LOGGER_ERROR( "movie slot '%s' type '%s' movie '%s' (please don't change parent it's danger)"
            , this->getName().c_str()
            , this->getType().c_str()
            , m_movieName.c_str()
        );
