...
@implementation iOSAmplitudePlugin
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if ([AppleBundle hasPluginConfig:@PLUGIN_BUNDLE_NAME] == NO) {
        IOS_LOGGER_ERROR( @"[iOSAmplitude] plugin config '%s' not found", PLUGIN_BUNDLE_NAME );
    ...
    if (apiKey == nil || apiKey.length == 0) {
        IOS_LOGGER_ERROR( @"[iOSAmplitude] plugin config '%s.ApiKey' is not specified", PLUGIN_BUNDLE_NAME );
