...
@implementation iOSAppLovinInterstitialDelegate
...
- (instancetype _Nullable) initWithAdUnitIdentifier:(NSString * _Nonnull)adUnitId advertisement:(id<iOSAdvertisementInterface> _Nonnull)advertisement {
    ...
    @try {
        ...
        IOS_LOGGER_MESSAGE(@"[Error] iOSAppLovinInterstitialDelegate invalid create MAInterstitialAd adUnitId: %@ exception: %@ [%@]"
            , adUnitId
            , ex.reason
            , ex.name
        );
    ...
    self.m_amazonLoader = [[iOSAppLovinInterstitialAmazonLoader alloc] initWithSlotId:amazonSlotId interstitialAd:self.m_interstitialAd];
#else
    [self loadAd];
#endif
...
- (void) didHideAd:(MAAd *)ad {
    ...
    id<iOSAdvertisementCallbackInterface> callback = [[iOSAppLovinPlugin sharedInstance] getAdvertisementInterstitialCallback];

    if (callback != nil) {
        [callback oniOSAdvertisementShowSuccess:ad.placement];
    }
...
- (void) didFailToDisplayAd:(MAAd *)ad withError:(MAError *)error {
    ...
    if (callback != nil) {
        [callback oniOSAdvertisementShowFailed:ad.placement withError:error.code];
    }
