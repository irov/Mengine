...
@implementation iOSAppLovinRewardedDelegate
...
- (instancetype _Nullable) initWithAdUnitIdentifier:(NSString * _Nonnull)adUnitId advertisement:(id<iOSAdvertisementInterface> _Nonnull)advertisement {
    ...
    @try {
        ...
        IOS_LOGGER_MESSAGE(@"[Error] iOSAppLovinRewardedDelegate invalid create MARewardedAd adUnitId: %@ exception: %@ [%@]"
            , adUnitId
            , ex.reason
            , ex.name
        );
    ...
    self.m_amazonLoader = [[iOSAppLovinRewardedAmazonLoader alloc] initWithSlotId:amazonSlotId rewardedAd:self.m_rewardedAd];
#else
    [self loadAd];
#endif
...
- (void) didRewardUserForAd:(MAAd *)ad withReward:(MAReward *)reward {
    ...
    if (callback != nil) {
        [callback oniOSAdvertisementUserRewarded:ad.placement withLabel:reward.label withAmount:reward.amount];
    }
}
