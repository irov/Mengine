...
@implementation iOSAppTrackingPlugin
...
- (void)updateTrackingStatus:(ATTrackingManagerAuthorizationStatus)status {
    switch(status) {
        case ATTrackingManagerAuthorizationStatusAuthorized: {
            ...
            self.m_idfa = [iOSAppTrackingPlugin makeIDFA];
        }break;
...
+ (instancetype)sharedInstance {
    static iOSAppTrackingPlugin * sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[iOSAppTrackingPlugin class]];
    });
...
- (void)onRunBegin {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::addScriptEmbedding<Mengine::iOSAppTrackingScriptEmbedding>( MENGINE_DOCUMENT_FUNCTION );
#endif
}

- (void)onStopEnd {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::removeScriptEmbedding<Mengine::iOSAppTrackingScriptEmbedding>();
#endif
}
