...
#define PLUGIN_BUNDLE_NAME "MengineiOSDatadogPlugin"

@implementation iOSDatadogPlugin

+ (instancetype)sharedInstance {
    static iOSDatadogPlugin *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[iOSDatadogPlugin class]];
    });
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    NSString * iOSDatadogPlugin_ClientToken = [AppleBundle getPluginConfigString:@PLUGIN_BUNDLE_NAME withKey:@"ClientToken" withDefault:nil];

    if (iOSDatadogPlugin_ClientToken == nil) {
    ...
    NSString * iOSDatadogPlugin_Service = [AppleBundle getPluginConfigString:@PLUGIN_BUNDLE_NAME withKey:@"Service" withDefault:bundleIdentifier];

    DDConfiguration * configuration = [[DDConfiguration alloc] initWithClientToken:iOSDatadogPlugin_ClientToken
    ...
    configuration.service = iOSDatadogPlugin_Service;

    NSString * iOSDatadogPlugin_Site = [AppleBundle getPluginConfigString:@PLUGIN_BUNDLE_NAME withKey:@"Site" withDefault:@"us1"];

    if ([iOSDatadogPlugin_Site isEqual:@"us1"] == YES) {
        configuration.site = [DDSite us1];
    } else if ([iOSDatadogPlugin_Site isEqual:@"us3"] == YES) {
        configuration.site = [DDSite us3];
    } else if ([iOSDatadogPlugin_Site isEqual:@"eu1"] == YES) {
        configuration.site = [DDSite eu1];
    } else {
        IOS_LOGGER_ERROR(@"plugin %@ Site %@ not found", @PLUGIN_BUNDLE_NAME, iOSDatadogPlugin_Site);
    ...
    DDLoggerConfiguration * loggerConfiguration = [[DDLoggerConfiguration alloc]
                                                   initWithService:iOSDatadogPlugin_Service
                                                   name:nil
