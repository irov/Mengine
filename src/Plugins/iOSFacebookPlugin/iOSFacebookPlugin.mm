...
- (instancetype)init {
    ...
    if( self != nil ) {
        ...
        self.m_shareDelegate = [[iOSFacebookShareDelegate alloc] initWithFacebook:self];
        self.m_userId = nil;
    }
...
- (void)onRunBegin {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::addScriptEmbedding<Mengine::iOSFacebookScriptEmbedding>( MENGINE_DOCUMENT_FUNCTION );
#endif
}

- (void)onStopEnd {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::removeScriptEmbedding<Mengine::iOSFacebookScriptEmbedding>();
#endif
}
...
#pragma mark - iOSFacebookInterface

+ (instancetype)sharedInstance {
    static iOSFacebookPlugin * sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[iOSFacebookPlugin class]];
    });
...
- (BOOL)login:(NSArray<NSString *> *)permissions {
    ...
    __weak iOSFacebookPlugin * weakSelf = self;
    ...
    [self.m_loginManager logInFromViewController:viewController configuration:configuration completion:^(FBSDKLoginManagerLoginResult * _Nullable result, NSError * _Nullable error) {
        iOSFacebookPlugin * strongSelf = weakSelf;
        ...
        id<iOSFacebookProviderInterface> provider = strongSelf.m_provider;
