...
@implementation iOSFirebaseRemoteConfigPlugin
...
+ (instancetype)sharedInstance {
    static iOSFirebaseRemoteConfigPlugin *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[iOSFirebaseRemoteConfigPlugin class]];
    });
...
- (void)onRunBegin {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::addScriptEmbedding<Mengine::iOSFirebaseRemoteConfigScriptEmbedding>( MENGINE_DOCUMENT_FUNCTION );
#endif
}

- (void)onStopEnd {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::removeScriptEmbedding<Mengine::iOSFirebaseRemoteConfigScriptEmbedding>();
#endif
}
