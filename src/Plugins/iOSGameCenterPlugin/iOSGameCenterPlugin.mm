...
#pragma mark - iOSGameCenterInterface

+ (instancetype)sharedInstance {
    static iOSGameCenterPlugin * sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[iOSGameCenterPlugin class]];
    });
    ...
    [self login:^(NSError * _error) {
        if (_error != nil) {
            ...
            [AppleDetail addMainQueueOperation:^{
                [callback oniOSGameCenterAuthenticate:NO];
            }];
        ...
        if (self.m_gameCenterAuthenticate == false) {
            ...
            [AppleDetail addMainQueueOperation:^{
                [callback oniOSGameCenterAuthenticate:YES];
            }];
        }
        ...
        [AppleDetail addMainQueueOperation:^{
            [callback oniOSGameCenterSynchronize:NO];
        }];
        ...
        [self loadCompletedAchievements:^(NSError * _error, NSArray * _completedAchievements) {
            ...
            if (callback != nil) {
                [AppleDetail addMainQueueOperation:^{
                    [callback oniOSGameCenterSynchronize:YES];
                }];
            }
        }] ;
    }];
...
- (void)onRunBegin {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::addScriptEmbedding<Mengine::iOSGameCenterScriptEmbedding>( MENGINE_DOCUMENT_FUNCTION );
#endif
}

- (void)onStopEnd {
#if defined(MENGINE_BUILD_MENGINE_SCRIPT_EMBEDDED)
    Mengine::Helper::removeScriptEmbedding<Mengine::iOSGameCenterScriptEmbedding>();
#endif
}
