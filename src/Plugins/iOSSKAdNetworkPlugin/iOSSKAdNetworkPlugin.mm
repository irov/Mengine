...
@implementation iOSSKAdNetworkPlugin

+ (instancetype)sharedInstance {
    static iOSSKAdNetworkPlugin * sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [iOSDetail getPluginDelegateOfClass:[iOSSKAdNetworkPlugin class]];
    });
...
- (BOOL)application:(UIApplication*)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    NSString * plistPath = [[NSBundle mainBundle] pathForResource:@MENGINE_IOS_SKADNETWORK_ADREVENUE_PLIST_NAME ofType:@"plist"];
    ...
    if (config == nil) {
        IOS_LOGGER_ERROR( @"can't load skadnetwork adrevenue plist: %@"
            , @MENGINE_IOS_SKADNETWORK_ADREVENUE_PLIST_NAME
        );
...
- (void)recordInstallation {
    ...
    [iOSSKAdNetworkDetail registerAppForAdNetworkAttribution:@"low"
...
- (void)onAdRevenue:(iOSAdRevenueParam *)revenue {
    ...
    [iOSSKAdNetworkDetail updatePostbackConversionValue:fine coarseValue:coarse lockWindow:NO completionHandler:^(NSError *error) {
        if (error != nil) {
            if ([iOSSKAdNetworkDetail isPostbackConversionUnknownError:error] == YES) {
                return;
            }
