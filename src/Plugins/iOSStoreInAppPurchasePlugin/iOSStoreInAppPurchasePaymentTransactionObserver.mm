...
@implementation iOSStoreInAppPurchasePaymentTransactionObserver
...
+ (instancetype _Nonnull)sharedInstance {
    static iOSStoreInAppPurchasePaymentTransactionObserver * sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[iOSStoreInAppPurchasePaymentTransactionObserver alloc] init];
    });
...
- (void)paymentQueue:(SKPaymentQueue *)queue updatedTransactions:(NSArray<SKPaymentTransaction *> *)transactions {
    ...
    {
        Mengine::iOSStoreInAppPurchasePaymentTransactionInterfacePtr paymentTransaction = [self.m_inAppPurchase makePaymentTransaction:skPaymentTransaction queue:queue];
            ...
            case SKPaymentTransactionStatePurchased:
            {
                if (skPaymentTransaction.originalTransaction != nil) {
                    ...
                    if ([self.m_nonconsumableIdentifiers containsObject:skPaymentTransaction.payment.productIdentifier] == YES) {
                        [[iOSStoreInAppPurchaseEntitlements sharedInstance] add:skPaymentTransaction.payment.productIdentifier];
                    }
...
- (void)paymentQueue:(SKPaymentQueue *)queue didRevokeEntitlementsForProductIdentifiers:(NSArray<NSString *> *)productIdentifiers {
    ...
    for (NSString * productIdentifier in productIdentifiers) {
        if ([productIdentifier rangeOfString:@".nonconsumable."].location != NSNotFound) {
            [[iOSStoreInAppPurchaseEntitlements sharedInstance] remove:productIdentifier];
        }
    }
