#include "RenderImage.h"
#include <assert.h>

D3D9RenderImage::D3D9RenderImage(LPDIRECT3DDEVICE9 _dev, const TextureDesc& _desc)
{ 
	mHaveAlpha = _desc.haveAlpha;

	if (FAILED(D3DXCreateTextureFromFileInMemoryEx(_dev, _desc.buffer, _desc.size,
                                    D3DX_DEFAULT, D3DX_DEFAULT,
                                    1,                  
                                    0,                  
									_desc.haveAlpha ? D3DFMT_A8R8G8B8 : D3DFMT_R8G8B8,    
                                    D3DPOOL_MANAGED,    
                                    D3DX_FILTER_NONE,   
                                    D3DX_DEFAULT,       
                                    0,                  
                                    NULL, NULL,
                                    &mTexPointer)))
    {
        assert(!"can't get TEXTURE desc!");
    }
 
    D3DSURFACE_DESC surfaceDesc;
 
    if(FAILED(mTexPointer->GetLevelDesc(0, &surfaceDesc)))
    {
        assert(!"can't get desc!");
    }

	mWidth = surfaceDesc.Width;
	mHeight = surfaceDesc.Height;

	mVertices[0].tcoor = mt::vec2f(0.0f, 0.0f);
	mVertices[1].tcoor = mt::vec2f(1.0f, 0.0f);
	mVertices[2].tcoor = mt::vec2f(1.0f, 1.0f);
	mVertices[3].tcoor = mt::vec2f(0.0f, 1.0f);

	mVertices[0].position = mt::vec3f(0.0f, 0.0f, 1.0f);
	mVertices[1].position = mt::vec3f(mWidth, 0.0f, 1.0f);
	mVertices[2].position = mt::vec3f(mWidth, mHeight, 1.0f);
	mVertices[3].position = mt::vec3f(0.0f, mHeight, 1.0f);

	mVertices[0].color = 0xFFFFFFFF;
	mVertices[1].color = 0xFFFFFFFF;
	mVertices[2].color = 0xFFFFFFFF;
	mVertices[3].color = 0xFFFFFFFF;
}
  
D3D9RenderImage::~D3D9RenderImage()
{
}

float	D3D9RenderImage::getWidth() const
{
	return mWidth;
}

float	D3D9RenderImage::getHeight() const
{
	return mHeight;
}

const D3D9Vertex*	D3D9RenderImage::_getD3D9V4() const
{
	return mVertices;
}

LPDIRECT3DTEXTURE9	D3D9RenderImage::_getTexPointer() const
{
	return mTexPointer;
}

bool	D3D9RenderImage::_isAlpha() const
{
	return mHaveAlpha;
}

