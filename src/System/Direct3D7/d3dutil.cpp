...
//-----------------------------------------------------------------------
LPDIRECTDRAW7 D3DUtil_GetDirectDrawFromDevice( LPDIRECT3DDEVICE7 pd3dDevice )
{
...
/* No longer work in Dx7
//-----------------------------------------------------------------------
// Name: D3DUtil_GetDeviceMemoryType()
// Desc: Retreives the default memory type used for the device.
//-----------------------------------------------------------------------
DWORD D3DUtil_GetDeviceMemoryType( LPDIRECT3DDEVICE7 pd3dDevice )
{
    D3DDEVICEDESC7 ddHwDesc;
    if( FAILED( pd3dDevice->GetCaps( &ddHwDesc ) ) )
        return 0L;
...
//-----------------------------------------------------------------------
DWORD D3DUtil_GetDisplayDepth( LPDIRECTDRAW7 pDD4 )
{
    ...
    if( NULL == pDD4 )
    {
        ...
        HRESULT hr = pDD1->QueryInterface( IID_IDirectDraw7, (VOID**)&pDD4 );
        pDD1->Release();
        if( FAILED(hr) )
            return 0L;
    }
...
{
    TCHAR buffer[256];
	std::sprintf( buffer, "%s(%ld): ", strFile, dwLine );
    OutputDebugString( buffer );
    OutputDebugString( strMsg );

    if( hr )
    {
		std::sprintf( buffer, "(hr=%08lx)\n", hr );
        OutputDebugString( buffer );
    }
