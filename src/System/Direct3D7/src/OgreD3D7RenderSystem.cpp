...
namespace Ogre {
    ...
    D3DRenderSystem::D3DRenderSystem(HINSTANCE hInstance)
    {
		...
		LogManager::getSingleton().logMessage("D3D7: " + getName() + " created.");
...
//        OgreGuard( "D3DRenderSystem::~D3DRenderSystem" );
        ...
        D3DXUninitialize();
		LogManager::getSingleton().logMessage("D3D7: " + getName() + " destroyed.");
...
//        OgreUnguard();
    ...
    RenderWindow* D3DRenderSystem::initialise(bool autoCreateWindow, const String& windowTitle)
    {
        ...
        if (!mActiveDDDriver)
            OGRE_EXCEPT(Exception::ERR_INVALIDPARAMS, "Problems finding requested DirectDraw driver!",
        ...
        if (autoCreateWindow)
        {
            ...
            if (fullScreen)
            {
                ...
                if (!vid)
                    OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR, "Can't find requested video mode.",
    ...
    void D3DRenderSystem::setAmbientLight(float r, float g, float b)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error setting ambient light.", "D3DRenderSystem::setAmbientLight");
    }
    ...
    void D3DRenderSystem::setShadingType(ShadeOptions so)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error setting shading mode.", "D3DRenderSystem::setShadingType");
...
//        OgreGuard( "D3DRenderSystem::createRenderWindow" );
        ...
        if( mRenderTargets.find( name ) != mRenderTargets.end() )
        {
			...
			OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR,msg,"D3DRenderSystem::createRenderWindow");
        }
	...
	MultiRenderTarget * D3DRenderSystem::createMultiRenderTarget(const String & name)
	{
    ...
    void D3DRenderSystem::setD3DLight(size_t index, Light* lt)
    {
        ...
        if (!lt)
        {
            ...
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Unable to disable light.", "D3DRenderSystem::setD3DLight");
        }
        else
        {
            switch (lt->getType())
            ...
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Unable to set light details", "D3DRenderSystem::setD3DLight");

            hr = mlpD3DDevice->LightEnable(index, TRUE);
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Unable to enable light.", "D3DRenderSystem::setD3DLight");
        }
    ...
    void D3DRenderSystem::_setWorldMatrix(const Matrix4 &m)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Cannot set D3D world matrix",
    ...
    void D3DRenderSystem::_setViewMatrix(const Matrix4 &m)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Cannot set D3D view matrix",
    ...
    void D3DRenderSystem::_setProjectionMatrix(const Matrix4 &m)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Cannot set D3D projection matrix",
        ...
        TrackVertexColourType tracking)
    {
            ...
            shininess != lastShininess)
        {
            ...
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error setting D3D material.", "D3DRenderSystem::_setSurfaceParams");
    ...
    void D3DRenderSystem::_setTexture(size_t stage, bool enabled, const String &texname)
    {
        ...
        if (enabled && !dt.isNull())
        {
            ...
            if (pTex != mTexStageDesc[stage].pTex)
            {
                ...
                if (FAILED(hr))
                    OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Unable to set texture in D3D.", "D3DRenderSystem::_setTexture");
        ...
        else
        {
            ...
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Unable to disable texture in D3D.", "D3DRenderSystem::_setTexture");
        ...
        const Frustum* frustum)
    {
        ...
        if( FAILED( hr ) )
            OGRE_EXCEPT( Exception::ERR_RENDERINGAPI_ERROR, "Error setting texture coord calculation", "D3DRenderSystem::_setTextureCoordCalculation" );
    ...
    void D3DRenderSystem::_setTextureMatrix(size_t stage, const Matrix4& xForm)
    {
		...
		else
		{
			...
			if( FAILED( hr ) )
				OGRE_EXCEPT( Exception::ERR_RENDERINGAPI_ERROR, "Error setting texture matrix", "D3D9RenderSystem::_setTextureMatrix" );
	...
	void D3DRenderSystem::_setTextureCoordSet( size_t stage, size_t index )
	{
		...
		if( FAILED( hr ) )
			OGRE_EXCEPT( Exception::ERR_RENDERINGAPI_ERROR, "Unable to set texture coord. set index", "D3DRenderSystem::_setTextureCoordSet" );
        // Record settings
...
//        OgreGuard( "D3DRenderSystem::_beginFrame" );
        ...
        if (!mActiveViewport)
			OGRE_EXCEPT(Exception::ERR_INTERNAL_ERROR, "Cannot begin frame - no viewport selected.",
        ...
        if (FAILED(hr))
		{
			if (hr == DDERR_SURFACELOST || hr == DDERR_SURFACEBUSY)
			{
				...
				if (FAILED(hr = mlpD3DDevice->BeginScene()))
					OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error beginning frame after device restore: " 
						+ getErrorDescription(hr),
					"D3DRenderSystem::_beginFrame");
			}
			else
			{
				OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error beginning frame: " + getErrorDescription(hr),
				"D3DRenderSystem::_beginFrame");
			}
		}
        ...
        if (firstTime)
        {
            ...
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error enabling alpha blending option.",
                "D3DRenderSystem::_beginFrame");

            // Allow specular effects
            hr = __SetRenderState(D3DRENDERSTATE_SPECULARENABLE, TRUE);
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error enabling specular option.",
                "D3DRenderSystem::_beginFrame");
...
//        OgreUnguard();
    ...
    void D3DRenderSystem::_setCullingMode(CullingMode mode)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Unable to set D3D culling mode.",
    ...
    void D3DRenderSystem::_setDepthBufferCheckEnabled(bool enabled)
    {
        ...
        if (enabled)
        {
            ...
            if (FAILED(hr))
                OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error setting depth buffer test state.",
    ...
    void D3DRenderSystem::_setDepthBufferWriteEnabled(bool enabled)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error setting depth buffer write state.",
    ...
    void D3DRenderSystem::_setDepthBufferFunction(CompareFunction func)
    {
        HRESULT hr = __SetRenderState(D3DRENDERSTATE_ZFUNC, 
            convertCompareFunction(func));
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error setting depth buffer test function.",
    ...
    void D3DRenderSystem::_setDepthBias(ushort bias)
    {
        HRESULT hr = __SetRenderState(D3DRENDERSTATE_ZBIAS, 
            bias);
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error setting depth bias.",
    ...
    */
    ...
    void D3DRenderSystem::setLightingEnabled(bool enabled)
    {
        ...
        if (FAILED(hr))
            OGRE_EXCEPT(Exception::ERR_RENDERINGAPI_ERROR, "Error lighting status.", "D3DRenderSystem::setLightingEnabled");
    }
    //-----------------------------------------------------------------------
	...
	//---------------------------------------------------------------------
	void D3DRenderSystem::_setPointSpritesEnabled(bool enabled)
	{
		if (enabled)
		{
	...
	//---------------------------------------------------------------------
	void D3DRenderSystem::_setPointParameters(Real size, bool attenuationEnabled, 
		Real constant, Real linear, Real quadratic, Real minSize, Real maxSize)
	{

	}
	//---------------------------------------------------------------------
	void D3DRenderSystem::_setTexture(size_t stage, bool enabled, const TexturePtr &tex)
	{
		HRESULT hr;
		D3DTexturePtr dt = tex;
		if (enabled && !dt.isNull())
		{
			// note used
			dt->touch();
	...
	//---------------------------------------------------------------------
	void D3DRenderSystem::_setTextureBorderColour(size_t unit, const ColourValue& colour)
	{
		/*HRESULT hr;
	...
	//---------------------------------------------------------------------
	void D3DRenderSystem::_setTextureMipmapBias(size_t unit, float bias)
	{

	}
	//---------------------------------------------------------------------
	void D3DRenderSystem::_setDepthBias(float constantBias, float slopeScaleBias)
	{

	}
	//---------------------------------------------------------------------
	VertexElementType D3DRenderSystem::getColourVertexElementType(void) const
	{
		return VET_COLOUR_ARGB;
	}
	//---------------------------------------------------------------------
	void D3DRenderSystem::_convertProjectionMatrix(const Matrix4& matrix, Matrix4& dest, bool forGpuProgram)
	{
		dest = matrix;
	...
	//---------------------------------------------------------------------
	void D3DRenderSystem::bindGpuProgramPassIterationParameters(GpuProgramType gptype)
	{
