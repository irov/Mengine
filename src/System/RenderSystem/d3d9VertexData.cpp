...
void	D3D9VertexData::lock(float* &_vertices, size_t _offset, size_t _count, size_t _vertexSizeInBytes)
{
	...
	HRESULT hr = m_VB->Lock( (UINT)(_offset * _vertexSizeInBytes), (UINT)(_count * _vertexSizeInBytes), (void**)&_vertices, D3DLOCK_NOOVERWRITE );
	if(hr != S_OK)
	{
		assert(0);
	}
...
void	D3D9VertexData::createVertexBuffer(size_t _count, size_t _vertexSizeInBytes, size_t _start)
{
	...
	HRESULT hr = m_pd3dDevice->CreateVertexBuffer( (UINT)(_count * _vertexSizeInBytes),
                                        0, 0,
