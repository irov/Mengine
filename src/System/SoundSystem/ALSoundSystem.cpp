...
bool ALSoundSystem::initialize( Menge::LogSystemInterface* _logSystem )
{
	...
	if( !m_device )
	{
		LOG_ERROR( "OpenAL failed to open device. Sound will be disabled" );
		return false;
	}
	...
	if( !m_context || alcGetError( m_device ) != ALC_NO_ERROR) 
	{
		...
		alcCloseDevice( m_device );
		LOG_ERROR( "OpenAL failed to create context. Sound will be disabled" );
		return false;
	} 
	...
	m_initialized = true;
	LOG( "OpenAL sound system initialized successfully" );
	return true;
}
//////////////////////////////////////////////////////////////////////////
