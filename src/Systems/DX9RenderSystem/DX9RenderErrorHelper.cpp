...
namespace Mengine
{
    namespace Detail
    {
        ...
        bool logDX9Error( HRESULT _hr, const Char * _file, uint32_t _line, const Char * _function, const Char * _method, const String & _args )
        {
            ...
            LOGGER_VERBOSE_LEVEL( "dx9", LM_ERROR, LFILTER_NONE, LCOLOR_RED, _file, _line, _function, LFLAG_SHORT | LFLAG_FILESTAMP | LFLAG_FUNCTIONSTAMP )("call '%s(%s)' get error: %s (hr:%x)"
                , _method
                , _args.c_str()
                , message
                , (uint32_t)_hr
