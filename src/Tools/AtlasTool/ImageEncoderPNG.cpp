...
namespace Menge
{
	...
	unsigned int ImageEncoderPNG::encode( unsigned char* _buffer, const CodecDataInfo* _bufferDataInfo )
	{
		...
		int numElemBytes = 4;
		if( dataInfo->format == PF_A8R8G8B8 )
		{
			numElemBytes = 4;
		}
		else if( dataInfo->format == PF_R8G8B8 )
		{
			numElemBytes = 3;
		}
		...
		if( info_ptr == NULL )  
		{
			printf( "PNG encoder error: Can't create info structure" );
			return 0;
		}
	...
	bool ImageEncoderPNG::initializeEncoder_()
	{
		...
		if( m_png_ptr == NULL )  
		{
			printf( "PNG encoder error: Can't create write structure" );
			return false;
		}
