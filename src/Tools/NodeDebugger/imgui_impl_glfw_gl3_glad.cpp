...
void ImGui_ImplGlfwGL3_NewFrame()
{
    ...
    io.DisplayFramebufferScale = ImVec2(w > 0 ? ((float)display_w / w) : 0.f, h > 0 ? ((float)display_h / h) : 0.f);

    // Setup time step
    double current_time =  glfwGetTime();
    io.DeltaTime = g_Time > 0.0 ? (float)(current_time - g_Time) : (float)(1.f / 60.f);
    g_Time = current_time;
    ...
    if (io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad)
    {
        ...
        #define MAP_BUTTON(NAV_NO, BUTTON_NO)       { if (buttons_count > BUTTON_NO && buttons[BUTTON_NO] == GLFW_PRESS) io.NavInputs[NAV_NO] = 1.f; }
        #define MAP_ANALOG(NAV_NO, AXIS_NO, V0, V1) { float v = (axes_count > AXIS_NO) ? axes[AXIS_NO] : V0; v = (v - V0) / (V1 - V0); if (v > 1.f) v = 1.0f; if (io.NavInputs[NAV_NO] < v) io.NavInputs[NAV_NO] = v; }
