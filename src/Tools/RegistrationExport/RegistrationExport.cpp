	...
	case WM_COMMAND:
		{
			...
			else if( LOWORD( wParam ) == ID_BTN_Ok )
			{
				...
				else
				{
					...
					RegSetValueEx( key, L"MENGINE_PROJECT_PATH", 0, REG_SZ, (LPBYTE)val_PathGame, wcslen( val_PathGame )*sizeof( wchar_t ) );
					...
					RegSetValueEx( key, L"MENGINE_PROJECT_PATH", 0, REG_SZ, (LPBYTE)val_PathGame, wcslen( val_PathGame )*sizeof( wchar_t ) );
					RegSetValueEx( key, L"MENGINE_PARAMS_PATH", 0, REG_SZ, (LPBYTE)val_PathParams, wcslen( val_PathParams )*sizeof( wchar_t ) );
					RegSetValueEx( key, L"MENGINE_ART_PATH", 0, REG_SZ, (LPBYTE)val_PathArt, wcslen( val_PathArt )*sizeof( wchar_t ) );
					RegCloseKey( key );
