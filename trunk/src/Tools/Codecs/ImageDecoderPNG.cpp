...
namespace Menge
{
	...
	unsigned int ImageDecoderPNG::decode( unsigned char* _buffer, unsigned int _bufferSize )
	{
		...
		{
			printf( "ImageDecoderPNG::decode Error: bad buffer size" );
			return 0;
		}

		if( m_bufferRowStride < m_rowStride )
		{
			printf( "ImageDecoderPNG::decode Error: bad buffer pitch" );
			return 0;
		}
	...
	bool ImageDecoderPNG::readHeader_()
	{
		...
		{
			printf( "PNG codec error: Bad or not PNG file" );
			return false;
		}
		...
		if( m_png_ptr == 0 )
		{
			printf( "PNG codec error: Can't create read structure" );
			return false;
		}
		...
		if( info_ptr == 0 ) 
		{
			printf( "PNG codec error: Can't create info structure" );
			return false;
		}
		...
		default:
			printf( "PNG codec error: PNG format not supported" );
			return false;
		}
